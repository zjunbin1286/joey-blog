...
### 2.1 内联首屏关键CSS
在打开一个页面，页面首要内容出现在屏幕的时间影响着用户的体验，而通过内联`css`关键代码能够使浏览器在下载完`html`后就能立刻渲染。而如果外部引用`css`代码，在解析`html`结构过程中遇到外部`css`文件，才会开始下载`css`代码，再渲染。所以，`CSS`内联使用使渲染时间提前
...
****
> 每文一句：有智慧的人未必先天就很聪明，反而更多的是通过后天毕生的努力。现在，我们这些正在求学的学生，当中，有很多人是认为自己先天不足，没办法学好，因此悲观泄气，无心向学。其实，这是大可不必的，只要勤奋努力，希望就在面前。
