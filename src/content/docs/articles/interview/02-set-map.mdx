...
`Map`类型是键值对的有序列表，而键和值都可以是任意类型。`Map`本身是一个构造函数，用来生成 `Map` 数据结构
...
m.set(hello, 'Hello CoderBin!') // 键是函数

console.log(m.get(hello)); // Hello CoderBin!
```
